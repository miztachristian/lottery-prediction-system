...
# Keras 3 backend must be chosen before importing keras
...
def load_csv(path):
    ...
    num_cols = needed + (['n6'] if 'n6' in df.columns else [])
    for c in num_cols:
...
def build_positional_encoding(max_len, d_model):
    ...
    return tf.constant(pe)  # constant is fine

class AddPositionalEncoding(layers.Layer):
    """Static slice (no tf.shape/ops on KerasTensor). Input length is fixed at build time."""
...
def sum6_aux_loss(y_pred_main):
    s = ops.sum(y_pred_main, axis=-1)
    return ops.mean(ops.square(s - 6.0))
...
def build_model(K, F):
    ...
    x = layers.Dropout(DROPOUT)(x)
    main = layers.Dense(BALLS, activation="sigmoid", name="main")(x)
    ...
    model.add_loss(LAMBDA_SUM6 * sum6_aux_loss(main))
    model.compile(optimizer=keras.optimizers.Adam(2e-4),
                  loss={"main": main_loss, "reserve": res_loss},
...
def train_and_predict(df, K, tickets=8, epochs=40, batch=64, val_tail=24):
    ...
    model.fit(X_tr, {"main": y_main_tr, "reserve": y_res_tr},
